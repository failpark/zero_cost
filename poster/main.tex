...
% Authors: Daniel Borgs, Shiwei Cui, Tylan Yildirim
...
\title{Compile-Time vs.\ Runtime Trade-offs\\in Systems Programming}

\author{Daniel Borgs \and Shiwei Cui \and Tylan Yildirim}
...
\footercontent{
\large
  Introduction to Scientific Research and Writing -- Winter Semester 2025/26}
...
\addtobeamertemplate{headline}{}
{
	\begin{tikzpicture}[remember picture,overlay]
	  ...
	  {\includesvg[height=2.2cm,inkscapelatex=false]{logos/THWS.svg}};
	\end{tikzpicture}
}
...
\begin{column}{\colwidth}
	...
	\begin{block}{Background: What Is a State Machine?}
		...
		\begin{figure}
			\centering
			\includesvg[width=0.85\colwidth,inkscapelatex=false]{../paper/svg/state_machine.svg}
		\end{figure}
		
		\heading{The Verification Problem}
		
		How do we ensure programs follow these rules?
	...
	\begin{block}{Three Implementations}
		\large
		We implemented the same file handle state machine three ways:
		
\heading{A. Defensive C --- Safe but Slow}
\begin{minted}{c}
int read_file(file_handle_t* h) {
	if (h->state != STATE_OPEN)   // runtime check
		return -1;				 // branch if invalid
...
\end{minted}
		...
		\heading{B. Minimal C --- Fast but Unsafe}
\begin{minted}{c}
int read_file(file_handle_t* h) {
	h->data = 42;	// no check -- undefined behavior if invalid
	return h->data;
}
\end{minted}

\heading{C. Rust Typestate --- Safe AND Fast}
\begin{minted}{rust}
...
// read() only exists for FileHandle<Open>
// Calling on FileHandle<Closed> = compile error
\end{minted}
\end{block}
...
\begin{column}{\colwidth}
	...
	\begin{block}{Results: Assembly Comparison}
		...
		\begin{figure}
			\centering
			\includegraphics[width=0.7\colwidth]{../paper/figures/overhead_ratio.pdf}
		\end{figure}
	\end{block}

	\begin{block}{Trade-off Thinking in Computer Science}
		\large
		\heading{A Unifying Principle}
		
		The compile-time vs.\ runtime trade-off appears throughout CS. Doing work \textit{earlier} often saves work \textit{later}.
		
		\begin{figure}
			\centering
			\includesvg[width=0.95\colwidth,inkscapelatex=false]{../paper/svg/simple_spectrum.svg}
		\end{figure}
		
		\heading{Three Examples from Our Group}
		
		\begin{table}[]
			\renewcommand{\arraystretch}{1.4}
			\centering
			\begin{tabular}{l|l|l}
				\textbf{Topic} & \textbf{Early Work} & \textbf{Late Savings} \\
				\hline
				Rust Typestate & Compiler verifies state & No runtime checks \\
				Huffman Coding (Tylan) & Build frequency tree & Shorter transmission \\
				Database Indexes (Shiwei) & B-Tree write overhead & Faster reads \\
			\end{tabular}
		\end{table}
	\end{block}
	...
	\begin{block}{References}
		\footnotesize
		\begin{enumerate}
			\item CWE -- 2024 CWE Top 10 KEV Weaknesses. \texttt{cwe.mitre.org}
			...
			\item Google Security Blog: ``Memory safe languages in Android 13'' (2022)
		\end{enumerate}
	\end{block}
