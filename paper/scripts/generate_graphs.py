...
def graph2_instruction_breakdown():
	...
	bars1 = ax.bar(x - width, defensive_c, width, label='Defensive C',
	               color=COLORS['defensive'], edgecolor='black', linewidth=0.5)
	bars2 = ax.bar(x, minimal_c, width, label='Minimal C',
	               color=COLORS['minimal'], edgecolor='black', linewidth=0.5)
	bars3 = ax.bar(x + width, rust, width, label='Rust',
