...
\usepackage{hyperref}
\hypersetup{
	pdftitle={Zero-Cost State Management: Comparing Rust Typestate and C Runtime Checks at the Assembly Level},
	pdfauthor={Daniel Borgs}
}
...
Rust provides zero-cost abstractions: compile-time verification without runtime penalty.
This paper aims to investigate whether Rust's typestate pattern produces assembly identical to unsafe C code.
...
\begin{IEEEkeywords}
memory safety, typestate, Rust, C, zero-cost abstractions, compile-time verification, ARM64, assembly analysis
\end{IEEEkeywords}
...
State management---ensuring operations occur only when preconditions are met\ \cite{google_paper}---requires explicit runtime checks in C.
In C, state is typically tracked with an enum field, and each operation checks this field before proceeding,
introducing conditional branches with associated overhead.
...
Many higher-level languages enforce safety through garbage collection and runtime checks,
...
Generic type parameters are instantiated through monomorphization,
generating specialized machine code for each concrete type at compile-time rather than using runtime dispatch.
...
The central aim is to investigate whether compile-time state verifications eliminate runtime overhead while preventing invalid state transitions.
...
and zero-sized---needing no memory allocated---\texttt{PhantomData} markers\ \cite{rust_reference_phantomdata}.
...
To the author's knowledge, no prior study has compared the assembly output of typestate implementations against equivalent C code at the instruction level.
This paper tries to address that gap through assembly-level analysis of concrete implementations.

\section{Methodology}

The complete source code for all implementations, build scripts, and generated assembly is available at \url{https://github.com/failpark/zero_cost/}.
...
Each implementation was compiled at -O2 optimization level---the industry standard\ \cite{red_hat_gcc_O_flag}---: Apple Clang 17.0.0 for C, and rustc 1.91.1 with Rust edition 2024 for Rust.
...
The C implementations were compiled to object files (\texttt{gcc -O2 -c}).

This study is limited to ARM64 architecture; results on other platforms such as x86-64 may differ.

Instruction counts were collected using a Python script that parses assembly output and categorizes instructions
by type (branch, compare, memory, arithmetic, control), ensuring reproducible and systematic analysis.
...
The label \texttt{error:} is a symbolic marker, not an executed instruction and thus not counted.
...
\begin{tabular}{lcccc}
\hline
\textbf{Function} & \textbf{Defensive C} & \textbf{Minimal C} & \textbf{Rust} & \textbf{Overhead} \\
\hline
\hline
\texttt{get\_data} & 7 & 2 & 2 & 3.5$\times$ \\
\texttt{open} & 9 & 2 & 2 & 4.5$\times$ \\
\texttt{read} & 11 & 4 & 2 & 5.5$\times$ \\
\texttt{close} & 8 & 3 & 2 & 4.0$\times$ \\
\hline
\end{tabular}
\end{table}

Across all functions, Rust and minimal C produce instruction-identical assembly, while defensive C requires 3.5--5.5$\times$ the instructions.
...
The typestate pattern extends principle beyond memory safety to protocol correctness.
...
Most importantly, the typestate pattern may increase type complexity and may impact compile times or code readability.
...
These findings suggest that for certain patterns, the traditional safety-performance tradeoff can be avoided.
With sufficiently expressive type systems, safety can become a compile-time property with zero runtime cost.
Future work should examine x86-64 platforms, more complex state machines, and runtime performance under real workloads.
