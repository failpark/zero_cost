...
\begin{lstlisting}[language=C, caption={Defensive C state management}]
typedef enum {
...
\begin{lstlisting}[language=C, caption={Minimal C without state checks}]
typedef struct {
    int data;
} file_handle_t;
...
\begin{lstlisting}[language=Rust, caption={Rust typestate pattern}]
struct Closed;
struct Open;
struct Readable;
