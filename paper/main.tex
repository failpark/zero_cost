...
\begin{abstract}
Memory safety vulnerabilities—use-after-free, buffer overflows, out-of-bounds write—are at the top of known exploited vulnerabilities\cite{cwe_2025}. System programming languages like C, which allow for low-level memory control, are needed to write performance critical code.
...
Higher-level languages like Java enforce safety through garbage collection and runtime checks. While used in some application contexts, their overhead typically makes them unsuitable for bare-metal systems programming such as operating system kernels.
...
The type parameters used in typestate patterns are erased during compilation through monomorphization\cite{rustonomicon}.
A \texttt{PhantomData<State>} field occupies zero bytes\cite{rust_reference_phantomdata}; it exists only to satisfy the type checker.
...
Session types\cite{honda_types_1993} provide a complementary approach for communication protocols.
Where typestate tracks individual object states, session types specify valid message sequences between parties.
Both approaches encode valid sequences in the type system.
...
\subsubsection{Beyond Memory Safety: Protocol Correctness}

The typestate pattern extends beyond memory safety to other vulnerability classes in the 2025 CWE Top 25:

\begin{itemize}
	\item \textbf{Missing Authorization (CWE-862, \#4)}: Authorization can be encoded as type transitions. A \texttt{Request<Unauthenticated>} must pass through \texttt{authenticate()} to become \texttt{Request<Authenticated>} before accessing protected resources.

	\item \textbf{Path Traversal (CWE-22, \#6)}: User-supplied paths typed as \texttt{UserPath<Unvalidated>} require validation to produce \texttt{SafePath<Validated>} before file operations accept them.

	\item \textbf{OS Command Injection (CWE-78, \#9)}: Command execution can require \texttt{Argument<Sanitized>} type witnesses, preventing injection from unsanitized input.
\end{itemize}

Typestate prevents \textit{protocol violations}---forgetting to perform validation---rather than \textit{logic errors} in validation itself. The pattern ensures developers cannot skip security checks; it does not verify check correctness.
